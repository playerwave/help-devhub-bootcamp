<template>
  ...
  <q-page padding class="receipt-page">
    <!-- ช่องค้นหาชื่อลูกค้าและเบอร์โทร -->
    ...
    <q-dialog v-model="isModalOpen">
      <q-card style="min-width: 450px; border-radius: 10px">
        ...
        >
          ...
          <p><strong>Customer Name:</strong> {{ selectedReceipt?.customer?.name }}</p>
          <p><strong>Customer Phone:</strong> {{ selectedReceipt?.customer?.phone }}</p>
        </q-card-section>
...
const openReceiptDetails = (receipt: Receipt) => {
  selectedReceipt.value = receipt; // กำหนดข้อมูลใบเสร็จที่เลือก
  isModalOpen.value = true; // เปิด modal เพื่อแสดงรายละเอียด
};

// ฟังก์ชันค้นหาข้อมูลในใบเสร็จ
const searchReceipts = () => {
  // จะฟิลเตอร์ข้อมูลโดยใช้ computed ที่เราได้ประกาศไว้
};

// ฟิลเตอร์ใบเสร็จที่เลือกช่วงเวลาและการค้นหา
const formattedReceipts = computed(() => receiptStore.formattedReceipts);

const filteredReceipts = computed(() => {
  let filtered = formattedReceipts.value;

  // ฟิลเตอร์ตามชื่อและเบอร์โทร
  if (searchName.value) {
    filtered = filtered.filter((receipt) =>
      receipt.customer?.name.toLowerCase().includes(searchName.value.toLowerCase())
    );
  }

  if (searchPhone.value) {
    filtered = filtered.filter((receipt) =>
      receipt.customer?.phone.toLowerCase().includes(searchPhone.value.toLowerCase())
    );
  }
